...
// credential is absent, the onboarding modal hosts the credential step, and
...
describe.skipIf(MODE === 'record')('web e2e: first-run DeepSeek credential setup', () => {
  ...
  it('never paints the takeover chrome on a configured reload, even with the settings join held open', async () => {
    ...
    // yet it must load its private fact before deciding not to show. Dialog
    // chrome lives inside each visible branch, so the deciding window paints
    // and blocks nothing. Holding settings/describe widens that window from
    // loopback RTT scale to a deterministic hundreds of milliseconds, removing
    // all timing dependence from the sampler assertions below.
    //
