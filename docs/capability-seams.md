...
| `ctx.taskService` | `core` | `api-task-service` | - | - | - | Submits one Agent session per task over the webserver and projects the session log as SSE progress, queryable results, cancellation, and completion webhooks. |
