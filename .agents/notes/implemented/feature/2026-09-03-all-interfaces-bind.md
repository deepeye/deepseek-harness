...
- **The browser-trust fence stays explicit.** An all-interfaces bind auto-trusts only the machine's own non-internal IPv4 literals (`resolveLanTrust`); a public hostname or address must be named with `--trusted-host`, preserving the DNS-rebinding protection. The all-interfaces bind never means "trust any Host header."
