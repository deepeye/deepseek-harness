...
**由同一个既有 client Cordis 插件持有已发布步骤。** `ui-settings-models` 在 `settings.onboarding` 中以顺序 `0` 注册 `deepseek-official`；原先在其之前以顺序 `-100` 注册的 `welcome-notice` 步骤已移除（见[移除说明](../simplification/2026-09-04-retire-welcome-notice-onboarding-step.zh.md)）。外壳仍然只挂载第一个未完成条目，因此弹窗不会堆叠。不新增 client 包或插件配置行。
...
**欢迎声明已移除。** 其版本化文案（`onboarding-copy.ts`）、`ui-onboarding.welcomeNoticeVersion` 持久化字段与步骤注册均已删除；见[移除说明](../simplification/2026-09-04-retire-welcome-notice-onboarding-step.zh.md)。声明所复用的 Host 设置与凭据边界不变。
...
新的回环 profile 仅当没有任何可用提供方时才会看到行内 DeepSeek 密钥弹窗；原先在其之前的内测声明已移除（见[移除说明](../simplification/2026-09-04-retire-welcome-notice-onboarding-step.zh.md)）。secret 仍以只写方式存入 `.credentials.yaml`，已就绪或无法修复的部署在加载判定期间不会渲染任何引导框架。Models 包现在同时持有产品引导展示与提供方配置；README 和浏览器覆盖明确记录了这项扩展后的职责。本决策原在历史上的[全屏内测声明移除](../../archived/simplification/2026-08-13-remove-first-run-beta-notice.md)之后恢复简洁的测试阶段声明，但不恢复那份声明中的遥测文案或接管式布局；恢复后的声明已由所链接的移除说明再次移除。
