...
The `auth` config field (default `true`) gates `/api` and the index behind the launch-token exchange and browser cookie above. `dsh --profile web --no-auth` sets it `false` for one invocation: the printed root URL carries no `?token=...`, `authorizeIndex` serves the index without challenge, and a `/api` request is rejected only by the Host/Origin trust fence (403) — never for a missing cookie (no 401). No signing secret is created or loaded, so the `client-connection/browser-session` credential record is untouched. The flag is for a loopback dev session or a deployment fronted by a TLS-terminating proxy that supplies its own authentication; combining `--no-auth` with `--host 0.0.0.0` exposes the agent to every reachable client and prints a stderr warning to that effect.
